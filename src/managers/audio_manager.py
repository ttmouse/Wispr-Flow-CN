    ...
    def start_recording(self) -> bool:
        ...
        with self._recording_lock:
            ...
            try:
                ...
                # 1. 保存当前系统音量
                self._save_system_volume()
                
                # 2. 启动音频捕获
                if self.audio_capture:
                    try:
                        self.audio_capture.start_recording()
                        self.logger.debug("音频捕获启动成功")
                    except Exception as e:
                        self.logger.error(f"音频捕获启动失败: {e}")
                        return False
                
                # 3. 启动音频捕获线程
                self._start_audio_capture_thread()
                
                # 4. 设置录音超时
                if self.recording_timer:
                    self.recording_timer.start(30000)  # 30秒超时
                ...
                self._handle_audio_error(f"启动录音失败: {e}")
                return False
