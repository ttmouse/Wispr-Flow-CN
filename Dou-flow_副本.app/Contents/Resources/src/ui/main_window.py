...
class MainWindow(QMainWindow):
    ...
    quit_requested = pyqtSignal()  # 添加退出请求信号
    ...
    def setup_title_bar(self):
        ...
        self.close_button.mousePressEvent = lambda e: self.quit_requested.emit()
    ...
    def _on_title_bar_mouse_move(self, event):
        ...
        if self._is_dragging and self._drag_start_pos is not None:
            ...
            if not self._drag_timer.isActive():
                self._drag_timer.start(16)  # 约60fps的更新频率
    ...
    def _apply_pending_move(self):
        """应用待处理的移动位置"""
        if self._pending_move_pos is not None and self._is_dragging:
            self.move(self._pending_move_pos)
